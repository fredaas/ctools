...
static uint32_t polynomial[] = {
    [HTABLE_POLY_NORMAL]              = 0x04C11DB7,
    [HTABLE_POLY_REVERSED]            = 0xEDB88320,
    [HTABLE_POLY_RECIPROCAL]          = 0xDB710641,
    [HTABLE_POLY_REVERSED_RECIPROCAL] = 0x82608EDB
};
...
Table * htable_init(int num_buckets)
{
    ...
    table->size = 0;
    table->buckets = (TableBucket **)malloc(num_buckets * sizeof(TableBucket *));
...
{
    void *dummy = NULL;
    int status = htable_get(self, key, key_size, (void *)&dummy);

    if (status != HTABLE_ENOITEM)
        return HTABLE_EEXISTS;

    TableBucket *new_bucket = (TableBucket *)malloc(sizeof(TableBucket));
    if (new_bucket == NULL)
        return HTABLE_ENOMEM;

    void *new_key = (void *)malloc(key_size);
    if (new_key == NULL)
    {
        free(new_bucket);
        return HTABLE_ENOMEM;
    ...
    uint32_t hash = crc32_hash(key, key_size);
    int index = hash % self->num_buckets;
...
int htable_get(Table *self, void *key, int key_size, void **value)
{
    uint32_t hash = crc32_hash(key, key_size);
    int index = hash % self->num_buckets;
    TableBucket *bucket = self->buckets[index];
...
TableBucket *htable_bucket(Table *self, int index)
{
    ...
    if ((index < 0) || (index >= self->num_buckets))
        return NULL;
...
int htable_clear(Table *self)
{
    ...
    {
        TableBucket *bucket = self->buckets[i];
        TableBucket *next = NULL;
        while (bucket != NULL)
        {
            next = bucket->next;
            free(bucket->key);
 ...
 *     POLY_RECIPROCAL
 *
 *     POLY_REVERSED_RECIPROCAL
 */
void crc32_generate(int type)
{
    int p = polynomial[type];
...
void crc32_dump(char *path)
{
    ...
    const int NUM_HEX_WIDTH = 6;

    fprintf(f, "0x00000000 ");

    for (int i = 1; i < 256; i++)
    {
        if ((i % NUM_HEX_WIDTH) == 0)
            fprintf(f, "\n");
        fprintf(f, "0x%08x ", crc32_table[i]);
    }
...
uint32_t crc32_hash(void *key, int key_size)
{
    ...
    uint32_t hash = 0xffffff;

    for (int i = 0; i < key_size; i++)
        hash = (hash >> 8) ^ crc32_table[data[i] ^ (uint8_t)hash];

    return (hash ^ 0xffffff);
}

/* Default CRC32 polynomial: HTABLE_POLY_REVERSED */
static uint32_t crc32_ieee802_3[256] = {
